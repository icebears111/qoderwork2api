...
// AES-128-CBC 加密身份 + MD5 请求签名。算法见 docs/api-reference.md §4。
package upstream
...
// NewCosySession 用 Cred 的持久指纹 + 当前 dt/drt 构建签名会话。
// dt/drt 变化（refresh）时需要重建（identity 内嵌这两个 token）。
// tempKey 是 16 个 ASCII 字符（与桌面端一致），不是随机字节。
func NewCosySession(c *cred.Cred, dt, drt string) (*CosySession, error) {
