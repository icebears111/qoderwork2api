...
| 思考模式 | `is_reasoning` 硬编码 false | 按模型开启：仅 `qwen3.8-flash`（qfmodel）开思考，支持 `low/medium/xhigh` 档位透传 |
...
- 🧠 **思考模式路由** — `qwen3.8-flash` 自动开思考（上游以 `source:"system"` 为真正开关，实测缺它 reasoning_content 永不下发）；客户端 `reasoning_effort` 归一为 low/medium/xhigh 后透传，非法值回落上游默认
